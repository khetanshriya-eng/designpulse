...
const LIME = "#d4ff3f";
const CREAM = "#fffaf0";
// "One template, two editions" palette (researched + adversarially verified
// 2026-07-19): Gmail iOS dark runs a full hue-preserving lightness inversion
// with no opt-out, so the AUTHORED light email uses cool porcelain/lavender
// neutrals whose inversion lands ON brand: porcelain card → ≈deep canvas
// #0f0a2a, navy text → night lavender, purple chips → #9e8cff family. Warm
// cream is reserved for text on purple/navy (its inversion stays readable).
const PAGE_BG = "#eeeaf7"; // lavender porcelain → inverts to deep violet
const CARD_BG = "#f7f5ff"; // cool porcelain → inverts to ≈ brand canvas
const GRAY = "#5c5470"; // summaries/date → inverts to mauve-gray, 7.8:1
const INTRO = "#3f3556"; // intro sits between title and summary in hierarchy
const BYLINE = "#6b6382";
const MONO = "'Courier New', Courier, monospace";

// Pixel wordmark PNG — the ONLY way lime + cream survive Gmail dark exactly
// (Gmail never repaints large images; ≥54px declared height defeats its
// small-image negative-invert). Generated by scripts/generate-email-wordmark.mjs.
const WORDMARK_URL = "https://www.designatorapp.com/email/wordmark.png";

// Night category label colors for clients that honor real dark CSS (Apple
// Mail / Outlook) — the site's vivid night stripe palette.
const NIGHT_CAT_COLORS: Record<string, string> = {
  "design-tools": "#d4ff3f",
  "ux-thinking": "#7c6bf5",
  inspiration: "#4fffb0",
  youtube: "#ff5252",
...
/**
 * Email-safe HTML digest — "one template, two editions" (verified strategy,
 * see the research audit 2026-07-19):
 *
 * - Gmail iOS dark full-inverts every declared color (no opt-out), so the
 *   authored light edition is designed so its INVERSION is the intended night
 *   edition: porcelain surfaces → brand deep canvas, navy text → night
 *   lavender, purple chips/buttons → the #9e8cff accent family.
 * - The only pixel-exact brand elements are (a) the wordmark PNG (images are
 *   never repainted at this size) on (b) a gradient-PINNED purple band, plus
 *   pinned lime keylines. Pinned surfaces carry NO live text — text on a
 *   pinned background still inverts (verified failure mode). Gradients are
 *   declared BOTH inline and as a class (Gmail Android corrupts inline-only
 *   gradients — email-bugs #135).
 * - Apple Mail / Outlook (~65% of opens) honor the prefers-color-scheme block
 *   and get the hand-authored TRUE night edition (deep canvas, lime chips).
 * - No decoy background-colors, no blend-mode dependence (refuted/fragile).
 */
export function renderDigestHtml(
  ...
  const rows = articles
    .map((a, i) => {
      ...
      // own nowrap line; (2) title; (3) summary; (4) source byline; (5) Read →.
      return `
      <tr>
        <td class="dm-cell" style="padding:26px 24px 28px;border-top:2px solid ${NAVY};">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <td class="dm-chip" style="background:${PURPLE};border:2px solid ${NAVY};font-family:${MONO};font-size:12px;line-height:1;font-weight:bold;color:${CREAM};padding:3px 9px;">${i + 1}</td>
              <td class="dm-cat-${esc(a.category)}" style="padding-left:12px;font-family:${MONO};font-size:11px;line-height:1;font-weight:bold;letter-spacing:2px;color:${catColor};white-space:nowrap;">${esc(cat.toUpperCase())}</td>
            </tr>
          </table>
          ...
          <div class="dm-byline" style="font-family:${MONO};font-size:12px;line-height:1;color:${BYLINE};">${esc(a.sourceName)} &nbsp;·&nbsp; ${esc(read)}</div>
          <div style="height:18px;line-height:18px;font-size:0;">&nbsp;</div>
          <a href="${esc(a.url)}" class="dm-btn" style="font-family:${MONO};font-size:13px;font-weight:bold;color:${CREAM};background:${PURPLE};padding:6px 12px;border:2px solid ${NAVY};text-decoration:none;">Read&nbsp;→</a>
        </td>
      </tr>`;
    })
    .join("");

  const nightCatRules = Object.entries(NIGHT_CAT_COLORS)
    .map(([slug, hex]) => `    .dm-cat-${slug} { color: ${hex} !important; }`)
    .join("\n");
...
<style>
  /* Gradient pins must ALSO live in a class: Gmail Android corrupts
     inline-only gradients (email-bugs #135). Pinned surfaces are text-free. */
  .gp-purple { background-image: linear-gradient(${PURPLE}, ${PURPLE}); }
  .gp-lime { background-image: linear-gradient(${LIME}, ${LIME}); }
  /* Real dark mode for clients that honor it (Apple Mail, Outlook): the
     hand-authored night edition — deep canvas, lime chips/CTAs, vivid
     category hues. Gmail ignores this block entirely; there the authored
     palette is DESIGNED to invert into the night edition on its own. */
  @media (prefers-color-scheme: dark) {
    .dm-page { background: #0a0620 !important; }
    .dm-card { background: #0f0a2a !important; border-color: #7668c2 !important; }
    .dm-edname { color: #f0ecff !important; }
    .dm-date { color: #9b93a8 !important; }
    .dm-intro { color: #c4bdd3 !important; }
    .dm-title { color: #f0ecff !important; }
    .dm-sum { color: #b6aecf !important; }
    .dm-byline { color: #9e8cff !important; }
    .dm-cell { border-color: #7668c2 !important; }
    .dm-chip { background: ${LIME} !important; color: ${NAVY} !important; }
    .dm-btn { background: ${LIME} !important; color: ${NAVY} !important; }
    .dm-ctabtn { color: ${LIME} !important; border-color: ${LIME} !important; }
${nightCatRules}
  }
</style>
</head>
  ...
  <tr>
    <td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" class="dm-card" style="max-width:600px;width:100%;background:${CARD_BG};border:3px solid ${NAVY};">
        <!-- Header band: gradient-pinned purple, image-only content — the one
             surface that stays brand-exact in every client and mode. -->
        <tr>
        ...
        <!-- Pinned lime keyline (text-free): exact lime everywhere. -->
        <tr>
        ...
        <!-- Edition identity lives on the CARD (unpinned): navy inverts into
             the night-lavender family, which reads designed, not accidental. -->
        <tr>
          <td style="padding:20px 24px 0;">
            <div class="dm-edname" style="font-family:${MONO};font-size:17px;line-height:1.3;font-weight:bold;color:${NAVY};">${esc(editionName)}</div>
            <div class="dm-date" style="font-family:${MONO};font-size:12px;line-height:1;color:${GRAY};margin-top:8px;">${esc(dateLabel)} &nbsp;·&nbsp; ${articles.length} stories</div>
          </td>
        </tr>
        <!-- Intro line (keeps the email from being 100% links) -->
        <tr>
          <td style="padding:16px 24px 20px;">
            <p class="dm-intro" style="font-family:${MONO};font-size:14px;line-height:1.65;color:${INTRO};margin:0;">Good morning. Here are today's design stories worth your time, picked from 75+ sources and summarized so you can catch up in five minutes.</p>
          </td>
        </tr>
        <!-- Stories -->
        ${rows}
        <!-- CTA band: unpinned navy — inverts to a lavender callout card. -->
        <tr>
          <td align="center" style="background:${NAVY};padding:24px;">
            <a href="${SITE}" class="dm-ctabtn" style="font-family:${MONO};font-size:14px;font-weight:bold;color:${CREAM};border:2px solid ${CREAM};padding:12px 20px;text-decoration:none;display:inline-block;">See today's full edition&nbsp;→</a>
          </td>
        </tr>
        <!-- Pinned lime keyline above the footer -->
        <tr>
          <td class="gp-lime" style="background-color:${LIME};background-image:linear-gradient(${LIME},${LIME});height:4px;line-height:4px;font-size:0;">&nbsp;</td>
        </tr>
